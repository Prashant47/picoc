...
enum LexToken LexCheckReservedWord(const char *Word)
{
     ...
     */
	    if(!strcmp(Word,ReservedWords[Count].SharedWord))
		    return ReservedWords[Count].Token;
    }
...
enum LexToken LexScanGetToken(struct LexState *Lexer, struct Value **Value)
{
    ...
    {
        *Value = &LexValue;
        while (Lexer->Pos != Lexer->End && isspace((char)*(Lexer->Pos)))
        {
            if (*Lexer->Pos == '\n')
            {
                Lexer->Line++;
...
//   printf("source text %s\n",Parser->SourceText); 
    ...
    //printf("Token after do while loop: %d\n",Token);
